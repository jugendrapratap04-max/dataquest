...
const mpProblems = [
  ...
  /* ============== 18. Logical, Compare and Rotate ============== */
  MP("mp-logical", "Easy", 359, "mp-split-digits", "Both Digits, as Numbers",
    "The byte at **2050H** has two hex digits. Store the **low** digit at **2060H** and the **high** digit at **2061H** — both as plain numbers from 0 to 15.\n\nMasking alone is not enough for the high digit. `ANI 0F0H` leaves it sitting in the top four columns, so A5H would give A0H rather than 0AH — the bits have to be **moved down** as well.\n\nFour `RRC` rotations move every bit down half a byte.",
    [{ input: "2050H = A5H", output: "2060H = 05H, 2061H = 0AH" }, { input: "2050H = 3CH", output: "2060H = 0CH, 2061H = 03H" }],
    "        LDA 2050H\n        ; low digit to 2060H, high digit to 2061H as a number\n" + HALT,
    "        LDA 2050H\n        ANI 0FH\n        STA 2060H\n        LDA 2050H\n        RRC\n        RRC\n        RRC\n        RRC\n        ANI 0FH\n        STA 2061H\n        HLT\n",
    [
      { memory: { [A50]: 0xa5 }, check: ["M:2060", "M:2061"] },
      { memory: { [A50]: 0x3c }, check: ["M:2060", "M:2061"] },
      { memory: { [A50]: 0xf0 }, check: ["M:2060", "M:2061"] },
    ],
    ["The low digit is one `ANI 0FH` away.",
      "For the high digit, rotate first and then mask — four `RRC` rotations bring it down to the bottom.",
      "Use `RRC`, not `RAR`: the circular rotate keeps all eight bits inside the byte.",
      "Mask after rotating, because the rotation wraps the low digit round to the top."],
    ["8085", "logical", "bitwise"]),

  MP("mp-logical", "Easy", 360, "mp-test-a-bit", "Is Bit 5 Set?",
    "Look at **bit 5** of the byte at **2050H**. Store **AAH** at **2060H** if it is set, and **55H** if it is clear.\n\nA single-bit test is a mask with one bit in it. `ANI 20H` clears everything except bit 5, so the result is either 20H or zero — and zero exactly when the bit was clear.\n\nBit 5 counting from zero at the right is the value 20H, not 05H.",
    [{ input: "2050H = A5H (bit 5 set)", output: "2060H = AAH" }, { input: "2050H = 0FH (bit 5 clear)", output: "2060H = 55H" }],
    "        LDA 2050H\n        ; mask off everything except bit 5, then branch\n" + HALT,
    "        LDA 2050H\n        ANI 20H\n        JZ CLEARED\n        MVI A, 0AAH\n        STA 2060H\n        HLT\nCLEARED: MVI A, 55H\n        STA 2060H\n        HLT\n",
    [
      { memory: { [A50]: 0xa5 }, check: ["M:2060"] },
      { memory: { [A50]: 0x20 }, check: ["M:2060"] },
      { memory: { [A50]: 0x0f }, check: ["M:2060"] },
    ],
    ["Bit 5 is the value 20H — bits count from zero at the right, so bit 5 is 2 to the power 5.",
      "`ANI 20H` leaves either 20H or 00H in the accumulator.",
      "`JZ` fires when the bit was clear.",
      "Do not test the carry here: `ANI` always clears it, so `JC` could never be taken."],
    ["8085", "logical", "branching"]),

  MP("mp-logical", "Medium", 361, "mp-set-and-clear", "Set Two Bits, Clear a Third",
    "Take the byte at **2050H**, **set** bits 7 and 0, **clear** bit 3, and leave every other bit exactly as it was. Store the result at **2060H**.\n\nSetting is OR with ones in the columns you want on. Clearing is AND with **zeros** in the columns you want off and ones everywhere else.\n\nSo the two masks are 81H and F7H, and the order does not matter because they touch different bits.",
    [{ input: "2050H = 00H", output: "2060H = 81H" }, { input: "2050H = FFH", output: "2060H = F7H" }],
    "        LDA 2050H\n        ; set bits 7 and 0, clear bit 3\n" + HALT,
    "        LDA 2050H\n        ORI 81H\n        ANI 0F7H\n        STA 2060H\n        HLT\n",
    [
      { memory: { [A50]: 0xa5 }, check: ["A", "M:2060"] },
      { memory: { [A50]: 0x00 }, check: ["A", "M:2060"] },
      { memory: { [A50]: 0xff }, check: ["A", "M:2060"] },
    ],
    ["Bits 7 and 0 are 80H and 01H, which together make the OR mask 81H.",
      "Bit 3 is 08H, so the AND mask is everything except that: F7H.",
      "An AND mask needs ones in every column you want to leave alone — zeros clear.",
      "Starting from FFH the answer is F7H, which is a good check that the OR did nothing there."],
    ["8085", "logical", "bitwise"]),

  MP("mp-logical", "Medium", 362, "mp-count-set-bits", "Count the Bits That Are On",
    "Count how many of the eight bits in the byte at **2050H** are set, and store the count at **2060H**.\n\nThe trick is the rotate: `RLC` drops the departing bit into the **carry**, so eight rotations pass every bit through it one at a time. `JNC` skips the count when the bit was zero.\n\nAfter eight circular rotations the accumulator is back to its original value, which is a useful property in itself.",
    [{ input: "2050H = A5H", output: "2060H = 04H" }, { input: "2050H = FFH", output: "2060H = 08H" }],
    "        LDA 2050H\n        MVI C, 08H\n        MVI B, 00H\n        ; rotate each bit through the carry and count the ones\n" + HALT,
    "        LDA 2050H\n        MVI C, 08H\n        MVI B, 00H\nLOOP:   RLC\n        JNC SKIP\n        INR B\nSKIP:   DCR C\n        JNZ LOOP\n        MOV A, B\n        STA 2060H\n        HLT\n",
    [
      { memory: { [A50]: 0xa5 }, check: ["A", "M:2060"] },
      { memory: { [A50]: 0xff }, check: ["A", "M:2060"] },
    ...
    ["`RLC` copies the bit that leaves the top into the carry, so `JNC` can test it.",
      "Keep the count in B — `INR B` does not disturb the carry, which is why it is safe here.",
