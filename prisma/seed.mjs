...
const S5 = [
  { t: "objectives", items: [
    "Read a <b>percentile</b> as a position in the queue, not as a score",
    "Split data into quarters with <b>Q1, the median and Q3</b>",
    "Measure spread with the <b>IQR</b>, and know why it survives extreme values",
    "Flag outliers with the <b>1.5 × IQR fence</b>, and know when that is the right tool",
  ]},
  { t: "hook", q: "One student says \"I got 96 percentile\". Another says \"I got 96 percent\". Have they done equally well?", why: "They have not said the same thing at all, and in most years the first has done far better. <b>96 percent</b> is a score: 96 marks out of 100, and it means nothing until you know how hard the paper was. <b>96 percentile</b> is a position: you finished ahead of 96% of everyone who sat the exam. In an entrance test with a million candidates, 96 percentile still leaves forty thousand people ahead of you, and the raw mark behind it might be 62. One number describes the paper, the other describes the queue — and confusing them is the single most common mistake in this topic." },
  { t: "def", term: "Percentile", en: "The k-th percentile is the value below which roughly k% of the data falls — so it describes a position within the dataset rather than a quantity measured from zero.", hi: "The median is simply the 50th percentile under another name. Percentiles work on any data you can put in order, including badly skewed data where the mean has stopped being useful." },
  { t: "note", variant: "key", html: "💼 <b>On the job:</b> engineering teams almost never report an average response time — they report <b>p95</b> and <b>p99</b>, the 95th and 99th percentiles. The reason is that an average hides the slow tail: a service can average a comfortable 200ms while one request in twenty takes four seconds, and it is those users who leave. Salary bands, growth charts and every entrance-exam rank in the country are percentiles too. When you see <code>p99 latency</code> in a job description, this lesson is what it means." },

  { t: "h2", n: "1", text: "A percentile is a position" },
  { t: "p", html: "To find where a value stands, count how many values sit below it and turn that count into a share of the whole." },
  { t: "code", file: "rank.py", code: "scores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\n\nbelow = len([s for s in scores if s < 79])\nprint(below)\nprint(round(below / len(scores) * 100, 1))", output: "7\n70.0" },
  { t: "p", html: "A score of 79 beat seven of the ten, so it sits at about the 70th percentile. Notice that 79 out of 100 as a <i>mark</i> and the 70th percentile as a <i>position</i> are two different facts about the same student, and neither one can be worked out from the other without the rest of the class." },

  { t: "h2", n: "2", text: "Quartiles cut the data into four" },
  { t: "p", html: "Three cuts divide sorted data into four equal groups. <b>Q1</b> is the 25th percentile, <b>Q2</b> is the 50th — the median — and <b>Q3</b> is the 75th. Python computes all three in one call." },
  { t: "code", file: "quartiles.py", code: "import statistics\n\nscores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\nq1, q2, q3 = statistics.quantiles(scores, n=4)\n\nprint(q1)\nprint(q2)\nprint(q3)\nprint(statistics.median(scores))", output: "56.5\n68.5\n80.5\n68.5" },
  { t: "p", html: "The last two lines print the same number, which is the check worth remembering: <b>Q2 and the median are the same thing</b>. A quarter of the class scored below 56.5, half below 68.5, three quarters below 80.5." },

  { t: "h2", n: "3", text: "The IQR, and why it does not flinch" },
  { t: "p", html: "The <b>interquartile range</b> is <code>Q3 - Q1</code>: the width of the middle half of the data. It throws away the top quarter and the bottom quarter before measuring, and that is exactly what makes it hard to fool." },
  { t: "code", file: "robust.py", code: "import statistics\n\nsalaries = [12, 15, 18, 21, 24, 27, 30, 33, 36, 40]\nq1, q2, q3 = statistics.quantiles(salaries, n=4)\nprint(round(statistics.mean(salaries), 2), round(statistics.pstdev(salaries), 2), q2, round(q3 - q1, 2))\n\nbig = salaries[:-1] + [100]\nb1, b2, b3 = statistics.quantiles(big, n=4)\nprint(round(statistics.mean(big), 2), round(statistics.pstdev(big), 2), b2, round(b3 - b1, 2))", output: "25.6 8.78 25.5 16.5\n31.6 23.95 25.5 16.5" },
  { t: "p", html: "One value changed, from 40 to 100. The <b>mean</b> climbed from 25.6 to 31.6 and the <b>standard deviation</b> nearly tripled, from 8.78 to 23.95. The <b>median</b> did not move at all, and neither did the <b>IQR</b> — both are still 25.5 and 16.5 to the decimal.<br/><br/>That is what <b>robust</b> means, and it is the whole argument for this lesson. The previous lesson's z-score is built on the mean and the standard deviation, so a single extreme value bends the very ruler you were going to measure it with." },
  { t: "viz", name: "box-plot" },
  { t: "p", html: "The box is Q1 to Q3 — the middle half — and the orange line inside it is the median. The panel uses the same ten numbers as the code above: nine fixed, and a tenth you control.<br/><br/>Drag the slider slowly from 20 upwards. Up to about 40 the box shuffles a little as the value takes its place in the order. <b>After 40 it stops dead.</b> Push the value to 60, to 80, to 100 and the box, the median and the fence do not move by a pixel — the quartiles have already stopped being able to see it. All that changes is the dot itself, which turns red the moment it crosses 58.5. Compare that with the standard deviation in the code above, which chased the same value from 8.78 to 23.95." },

  { t: "h2", n: "4", text: "The 1.5 × IQR fence" },
  { t: "p", html: "The standard rule for flagging an outlier builds a fence at <code>Q1 - 1.5 × IQR</code> below and <code>Q3 + 1.5 × IQR</code> above. Anything outside is worth a second look." },
  { t: "code", file: "fence.py", code: "import statistics\n\nsalaries = [12, 15, 18, 21, 24, 27, 30, 33, 36, 40]\nq1, q2, q3 = statistics.quantiles(salaries, n=4)\niqr = q3 - q1\n\nprint(round(q1 - 1.5 * iqr, 2))\nprint(round(q3 + 1.5 * iqr, 2))\n\nbig = salaries[:-1] + [100]\nprint([v for v in big if v < q1 - 1.5 * iqr or v > q3 + 1.5 * iqr])", output: "-7.5\n58.5\n[100]" },
  { t: "p", html: "The lower fence lands at -7.5, which no salary can reach, so nothing is ever flagged at the bottom of this dataset — a reminder that a fence is only as meaningful as the range the data could actually occupy. The upper fence is 58.5, and the 100 clears it comfortably." },
  { t: "note", variant: "warn", html: "<b>Flagged is not the same as wrong.</b> The 1.5 rule is a convention, not a law of nature — it comes from Tukey and it is tuned so that roughly 0.7% of genuinely normal data trips it. On a large dataset you should <i>expect</i> some points outside the fence with nothing wrong at all. Investigate what it flags; never let a script delete it." },

  { t: "think", q: "Why does the IQR survive an extreme value when the standard deviation does not?", a: "Because of what each one looks at.<br/><br/>The standard deviation is built from <b>every</b> deviation from the mean, and each one is squared before being averaged. Squaring means a value ten times further out contributes a hundred times as much, so one extreme number can dominate the entire calculation. It also drags the mean it is measured against, so the damage lands twice.<br/><br/>The IQR only asks where the 25% and 75% boundaries fall. Move the largest value from 40 to 100 and it is still the largest value — its <b>rank</b> is unchanged, and rank is all the quartiles read. You could push it to a million and Q3 would not notice.<br/><br/>The general principle: statistics built on <b>order</b> are robust; statistics built on <b>arithmetic over every value</b> are not." },
  { t: "analogy", concept: "IQR versus standard deviation", real: "Judging a competition", html: "Gymnastics and diving throw away the highest and lowest scores before averaging the rest. Nobody thinks this is a trick — it is there so that one judge with a grudge or a soft spot cannot decide the result on their own.<br/><br/>The IQR is the same instinct made into arithmetic. Discard the top quarter and the bottom quarter, then measure what is left in the middle. The standard deviation is the tournament that counts every judge equally, including the one who scored a perfect ten out of spite. Both are defensible; only one of them is hard to manipulate." },

  { t: "trace", intro: "Quartiles on five evenly spaced values. Work out what each name holds once the line has run.", code: "import statistics\nd = [10, 20, 30, 40, 50]\nq1 = statistics.quantiles(d, n=4)[0]\nq3 = statistics.quantiles(d, n=4)[2]\niqr = round(q3 - q1, 2)", steps: [
    { q: "After line 3, <code>q1</code> is", answer: "15.0", accept: ["15"], why: "With five values Python interpolates rather than picking one of them: the 25% boundary falls halfway between 10 and 20. Quartiles are positions in the order, and a position can land between two data points." },
    { q: "After line 4, <code>q3</code> is", answer: "45.0", accept: ["45"], why: "By the same symmetry, the 75% boundary sits halfway between 40 and 50. Notice that neither quartile is a value that appears in the list." },
    { q: "After line 5, <code>iqr</code> is", answer: "30.0", accept: ["30"], why: "45.0 minus 15.0. The middle half of this data spans 30 units, which on a range of 40 is wide — evenly spread data has a large IQR relative to its range, and clustered data has a small one." },
  ]},

  { t: "drills", intro: "One idea each. Write it yourself before you open the answer.", items: [
    { task: "Count how many scores fall below 67.", code: "scores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\nprint(len([s for s in scores if s < 67]))", out: "4" },
    { task: "Turn that count into a percentile.", code: "scores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\nprint(round(len([s for s in scores if s < 67]) / len(scores) * 100, 1))", out: "40.0" },
    { task: "All three quartiles in one call.", code: "import statistics\nscores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\nprint(statistics.quantiles(scores, n=4))", out: "[56.5, 68.5, 80.5]" },
    { task: "The IQR of those scores.", code: "import statistics\nq1, q2, q3 = statistics.quantiles([45, 52, 58, 61, 67, 70, 74, 79, 85, 91], n=4)\nprint(round(q3 - q1, 2))", out: "24.0" },
    { task: "The upper fence.", code: "import statistics\nq1, q2, q3 = statistics.quantiles([45, 52, 58, 61, 67, 70, 74, 79, 85, 91], n=4)\nprint(round(q3 + 1.5 * (q3 - q1), 2))", out: "116.5" },
    { task: "The lower fence.", code: "import statistics\nq1, q2, q3 = statistics.quantiles([45, 52, 58, 61, 67, 70, 74, 79, 85, 91], n=4)\nprint(round(q1 - 1.5 * (q3 - q1), 2))", out: "20.5" },
    { task: "Pick out the values beyond a fence you have already worked out at 150.", code: "print([v for v in [3, 40, 45, 52, 58, 61, 67, 70, 74, 300] if v > 150])", out: "[300]" },
    { task: "Confirm that Q2 and the median are the same number.", code: "import statistics\nscores = [45, 52, 58, 61, 67, 70, 74, 79, 85, 91]\nq1, q2, q3 = statistics.quantiles(scores, n=4)\nprint(statistics.median(scores) == q2)", out: "True" },
    { task: "Show the median ignoring an extreme value.", code: "import statistics\nprint(statistics.median([1, 2, 3, 4, 100]))", out: "3" },
    { task: "Show the standard deviation failing to ignore it.", code: "import statistics\nprint(round(statistics.pstdev([1, 2, 3, 4, 5]), 2), round(statistics.pstdev([1, 2, 3, 4, 100]), 2))", out: "1.41 39.01" },
    { task: "The p95 an engineering team would actually report.", code: "import statistics\nlat = list(range(1, 101))\nprint(round(statistics.quantiles(lat, n=100)[94], 2))", out: "95.95" },
  ]},

  { t: "mistakes", items: [
    { bad: "# \"I scored 90 percentile\" == \"I scored 90 percent\"", why: "A percentile is a rank and a percent is a score. 90 percentile means you finished ahead of 90% of the field, which in a large exam can sit on a raw mark in the fifties. They answer different questions and are never interchangeable.", fix: "# percentile -> position in the queue\n# percent    -> marks out of the total" },
    { bad: "q1, q3 = statistics.quantiles(data, n=4)", why: "<code>quantiles(data, n=4)</code> returns <b>three</b> cut points, not two, so this raises a ValueError about unpacking. The count of cuts is always n-1: four groups need three cuts between them.", fix: "q1, q2, q3 = statistics.quantiles(data, n=4)" },
    { bad: "iqr = statistics.quantiles(unsorted_raw)   # straight from a CSV column", why: "Quartiles are defined on ordered data. Python's <code>quantiles</code> sorts internally so this specific call is safe — but the same assumption written by hand, indexing into a list to find a quartile, silently returns nonsense on unsorted input.", fix: "iqr = statistics.quantiles(sorted(raw), n=4)   # explicit, and true either way" },
    { bad: "if value > q3 + 1.5 * iqr:\n    rows.drop(value)", why: "The fence is a flag, not a verdict. Real data has genuine extremes — the outage, the fraud, the record sale — and they are often the rows carrying the most information. Deleting them because a formula raised its hand is how a dataset gets tidied into telling you nothing.", fix: "if value > q3 + 1.5 * iqr:\n    review.append(value)" },
  ]},

  { t: "debug", intro: "This is meant to report the width of the middle half of the data — the IQR. It runs cleanly and returns 8.25, but the middle half of this data is twice that wide. Read it before opening the fix.", code: "import statistics\n\ndef iqr_of(values):\n    q1, q2, q3 = statistics.quantiles(values, n=4)\n    return round(q3 - q2, 2)\n\ndata = [12, 15, 18, 21, 24, 27, 30, 33, 36, 100]\nprint(iqr_of(data))", symptom: "reports 8.25, which is only the distance from the median up to Q3 — exactly half of the real interquartile range of 16.5", q: "The three quartiles are all computed correctly. So which two of them should the subtraction have used?", fix: "import statistics\n\ndef iqr_of(values):\n    q1, q2, q3 = statistics.quantiles(values, n=4)\n    return round(q3 - q1, 2)\n\ndata = [12, 15, 18, 21, 24, 27, 30, 33, 36, 100]\nprint(iqr_of(data))", why: "The interquartile range runs from <b>Q1 to Q3</b>, which is the middle 50% of the data. Subtracting <code>q3 - q2</code> measures only from the median upwards — the third quarter alone, 25% of the data rather than 50%.<br/><br/>Nothing crashes, because <code>q2</code> and <code>q1</code> are both perfectly good numbers and the subtraction is valid arithmetic either way. The answer is simply a different quantity wearing the IQR's name. Here it lands at exactly half the true width — 8.25 against 16.5 — which is close enough to look plausible on a dashboard and quite wrong enough to move every fence you build from it.<br/><br/>The habit that catches this: an IQR is a <b>range</b>, so ask which two boundaries it runs between before writing the minus sign. If the answer is not the first quartile and the third, it is not an IQR." },

  { t: "recap", items: [
    "A <b>percentile</b> is a position in the order — the 96th percentile beat 96% of the field, whatever the raw mark was",
    "<b>Q1, Q2, Q3</b> are the 25th, 50th and 75th percentiles, and Q2 is the median",
    "<code>IQR = Q3 - Q1</code> — the width of the middle half of the data",
    "The median and the IQR are <b>robust</b>: extreme values change their rank, not their position",
    "The fence is <code>Q1 - 1.5 × IQR</code> and <code>Q3 + 1.5 × IQR</code> — it flags for review, it does not condemn",
  ]},

  { t: "interview", items: [
    { level: "beginner", q: "What is the difference between a percentile and a percentage?", a: "A percentage is a score out of a total; a percentile is a rank within a group. Scoring 96 percent says you got 96 of 100 marks. Finishing at the 96th percentile says you beat 96% of the other candidates, and the raw mark behind it could be anything. One describes the paper, the other describes the field." },
    { level: "beginner", q: "What is the IQR and what does it tell you?", a: "It is Q3 minus Q1 — the range covered by the middle 50% of the data once the top and bottom quarters are set aside. It is a measure of spread, and it is preferred over the standard deviation when the data is skewed or has extreme values, because discarding the tails is precisely what makes it stable." },
    { level: "intermediate", q: "When would you report the median and IQR instead of the mean and standard deviation?", a: "Whenever the data is skewed or contains genuine extremes — salaries, house prices, response times, city populations. The mean is pulled towards a long tail and the standard deviation is inflated by it, so together they describe a distribution that nobody in the dataset actually experiences. Median and IQR describe the typical case. A quick test: if the mean and median are far apart, report the robust pair." },
    { level: "intermediate", q: "Why do engineering teams track p95 and p99 latency instead of the average?", a: "Because an average hides the tail, and the tail is where users leave. A service can average 200ms while one request in a hundred takes four seconds; the mean looks healthy and a percent of your traffic is having a terrible time. p95 and p99 report what the worst-served users actually experience, which is what an SLA is really promising." },
    { level: "advanced", q: "The 1.5 × IQR rule flags 40% of the rows in a column. What does that tell you, and what would you do?", a: "Not that 40% of the data is broken — the rule is calibrated so that around 0.7% of normal data trips it, so a figure that large is evidence about the distribution rather than about the rows. The likely explanations are a heavy right skew, where the fence sits far below the natural range of the upper tail, or a mixture of two different populations sharing one column, or a unit change partway through the data. I would plot it before deciding anything. If it is genuinely skewed, a log transform first, or a rule based on the median absolute deviation, is more honest than widening the multiplier until the count looks comfortable." },
  ]},
];
const S6 = [
  { t: "objectives", items: [
    "Read <b>covariance</b> as direction, and know why its size means nothing on its own",
    "Compute <b>correlation (r)</b> and read it on the fixed scale from −1 to +1",
    "Recognise the relationships r cannot see, including perfect ones",
    "Explain why <b>correlation is not causation</b>, using a confounder rather than a slogan",
  ]},
  { t: "hook", q: "In a city's records, the number of fire engines sent to a fire and the rupees of damage correlate at 0.99. Should the city send fewer engines?", why: "Obviously not — and the reason is the most important idea in this lesson. Both numbers are being driven by a third one nobody put in the table: <b>how big the fire is</b>. Big fires get more engines, and big fires do more damage. The engines and the damage move together beautifully without one causing the other at all.<br/><br/>A correlation is a statement about two columns moving in step. It says nothing whatever about which one is in charge, or whether either of them is. Confusing those two things has closed hospitals, cancelled drugs and cost companies a great deal of money, and it is the mistake this lesson exists to stop you making." },
  { t: "def", term: "Correlation coefficient (r)", en: "A number between −1 and +1 measuring how closely two variables move together in a straight line: +1 is a perfect upward line, −1 a perfect downward one, and 0 means no linear relationship at all.", hi: "It is covariance divided by both standard deviations, which is what strips out the units and forces the answer onto a fixed scale you can compare across any two datasets." },
  { t: "note", variant: "key", html: "💼 <b>On the job:</b> a correlation matrix is usually the first thing anyone runs on a new dataset. Two uses dominate. <b>Feature selection</b> — which columns actually move with the thing you are predicting. <b>Multicollinearity</b> — two input columns correlated at 0.95 are telling the model the same thing twice, which makes its coefficients unstable and unreadable. There is a third, less obvious one: a feature correlated with the target at 0.99 is usually not a triumph, it is <b>leakage</b>, and you should go and find out how the answer got into your inputs." },

  { t: "h2", n: "1", text: "Covariance: the right direction, the wrong units" },
  { t: "p", html: "Covariance asks whether two variables move the same way: when one is above its mean, is the other above its own? Positive means they rise together, negative means they move oppositely." },
  { t: "code", file: "cov.py", code: "import statistics\n\nhours = [1, 2, 3, 4, 5, 6, 7]\nscore = [2, 3, 3, 5, 5, 7, 8]\n\nprint(round(statistics.covariance(hours, score), 4))\nprint(round(statistics.correlation(hours, score), 4))", output: "4.6667\n0.9754" },
  { t: "p", html: "Positive, so more hours goes with a higher score. But is 4.67 a strong relationship or a weak one? The question has no answer, because covariance carries the units of both variables multiplied together — here it is measured in <i>hour-marks</i>, which is not a thing anybody has intuition about." },
  { t: "code", file: "units.py", code: "import statistics\n\nhours = [1, 2, 3, 4, 5, 6, 7]\nscore = [2, 3, 3, 5, 5, 7, 8]\nscore_paise = [s * 100 for s in score]\n\nprint(round(statistics.covariance(hours, score_paise), 4))\nprint(round(statistics.correlation(hours, score_paise), 4))", output: "466.6667\n0.9754" },
  { t: "p", html: "Nothing about the relationship changed — the second list is the same marks in smaller units. The covariance jumped a hundredfold, from 4.67 to 466.67, and the correlation did not move a digit. That is the whole case for correlation in two lines: <b>you can change the covariance just by changing the units</b>, so its magnitude cannot mean anything." },

  { t: "h2", n: "2", text: "Correlation: the same signal, made comparable" },
  { t: "p", html: "Divide the covariance by both standard deviations and the units cancel. What is left is trapped between −1 and +1 forever, whatever the two variables were measured in." },
  { t: "viz", name: "scatter-correlation" },
  { t: "p", html: "Press the three buttons and watch the cloud of points change shape. <b>Positive</b> gives +0.98 and a rising band; <b>Negative</b> gives −0.98 and a falling one; <b>No correlation</b> gives −0.04 and a scatter with no direction at all.<br/><br/>That third number is worth pausing on. It is not zero. Genuinely unrelated data almost never produces exactly zero — small non-zero correlations appear by chance all the time, especially with few points. \"No relationship\" in practice means <i>near</i> zero, and part of doing this well is knowing how near counts as near." },
  { t: "note", variant: "tip", html: "<b>A rough reading of r</b>, and it is only rough: below 0.3 is weak, 0.3 to 0.7 is moderate, above 0.7 is strong. The sign and the strength are separate facts — an r of −0.9 is a <b>strong</b> relationship, not a weak one. Beginners routinely rank −0.9 below +0.4 because the minus sign looks like less." },

  { t: "h2", n: "3", text: "What r cannot see" },
  { t: "p", html: "Correlation measures one specific thing: how well a <b>straight line</b> fits. A relationship that is perfect but curved is invisible to it." },
  { t: "code", file: "curve.py", code: "import statistics\n\nx = [-3, -2, -1, 0, 1, 2, 3]\ny = [9, 4, 1, 0, 1, 4, 9]\n\nprint(round(statistics.correlation(x, y), 4))", output: "0.0" },
  { t: "p", html: "Every value of <code>y</code> is exactly <code>x</code> squared. Knowing x tells you y with total certainty — there is no relationship more perfect than this one — and r reports <b>0.0</b>. Not approximately zero: exactly zero, because the curve rises on one side and falls on the other and the two halves cancel.<br/><br/>So r = 0 does not mean \"no relationship\". It means \"no <i>straight-line</i> relationship\". This is why the first thing you should do with an interesting correlation is plot it." },

  { t: "h2", n: "4", text: "Correlation is not causation" },
  { t: "p", html: "Back to the hook, with numbers. A <b>confounder</b> is a third variable driving both of the two you are looking at." },
  { t: "code", file: "cause.py", code: "import statistics\n\nfire_size = [1, 2, 3, 4, 5, 6]\nengines = [1, 2, 2, 3, 4, 5]\ndamage = [10, 22, 28, 44, 51, 63]\n\nprint(round(statistics.correlation(engines, damage), 4))\nprint(round(statistics.correlation(fire_size, engines), 4))\nprint(round(statistics.correlation(fire_size, damage), 4))", output: "0.9853\n0.9805\n0.9952" },
  { t: "p", html: "Engines and damage correlate at 0.9853, which is the alarming-looking number that started the lesson. But fire size correlates with the damage at <b>0.9952</b> — more strongly than the engines do — and with the engines at 0.9805. The hidden variable explains both columns better than they explain each other.<br/><br/>Nothing in the arithmetic could have told you this. The only reason we found the confounder is that somebody knew what fires are. <b>Correlation is a question, not an answer</b>, and the answer comes from understanding the subject." },

  { t: "think", q: "If correlation cannot demonstrate causation, why is it worth computing at all?", a: "Because it narrows the field, and that is a genuinely valuable thing to do.<br/><br/>The logic only runs one way. A strong correlation does not prove a cause, but a <b>near-zero</b> correlation is real evidence that there is no simple linear cause to find — so you can stop looking there and spend your time elsewhere. Out of a hundred candidate columns, correlation tells you which handful are worth a proper investigation.<br/><br/>It is also the honest starting point for the methods that <i>can</i> establish causation: a controlled experiment, an A/B test, or a natural experiment where something outside the system moved one variable and not the other. Every one of those begins with somebody noticing a correlation and refusing to stop there." },
  { t: "analogy", concept: "Correlation", real: "Two people arriving together every morning", html: "You notice that two colleagues walk into the office within a minute of each other, every single day for a year. The correlation is nearly perfect, and it is a real pattern — you could bet on it and win.<br/><br/>What you cannot tell from the pattern alone is why. Perhaps they are neighbours who share a car. Perhaps one waits for the other. Or perhaps — and this is the confounder — they simply both catch the 8:40 train, and neither has ever spoken to the other.<br/><br/>The correlation is genuine in all three cases and identical in all three cases. Working out which one is true requires going and asking them, not staring harder at the arrival times." },

  { t: "trace", intro: "Covariance, correlation and r squared on data that rises but wobbles. Work out what each name holds once the line has run.", code: "import statistics\na = [1, 2, 3, 4, 5]\nb = [2, 4, 5, 4, 6]\ncov = round(statistics.covariance(a, b), 2)\nr = round(statistics.correlation(a, b), 2)\nr2 = round(r ** 2, 2)", steps: [
    { q: "After line 4, <code>cov</code> is", answer: "2.0", accept: ["2"], why: "Positive, so b tends to be above its mean when a is. That is the entire message: the value 2.0 is measured in a-units times b-units, so its size tells you nothing until it has been divided by both standard deviations." },
    { q: "After line 5, <code>r</code> is", answer: "0.85", why: "Strong and positive, but not perfect — the dip at the fourth point is what keeps it off 1.0. Unlike the covariance above it, this number can be compared directly against the r of any other pair of columns anywhere." },
    { q: "After line 6, <code>r2</code> is", answer: "0.72", why: "r squared, usually written R². Read it as the share of b's variation that a straight line on a accounts for — about 72% here, with the remaining 28% unexplained. Squaring also discards the sign, which is why R² alone never tells you which way the relationship runs." },
  ]},

  { t: "drills", intro: "One idea each. Write it yourself before you open the answer.", items: [
    { task: "Correlation of a perfect rising line.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5], [2,4,6,8,10]), 2))", out: "1.0" },
    { task: "Correlation of a perfect falling line.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5], [10,8,6,4,2]), 2))", out: "-1.0" },
    { task: "Covariance of two short lists.", code: "import statistics\nprint(round(statistics.covariance([1,2,3], [4,5,6]), 2))", out: "1.0" },
    { task: "Correlation of the hours-and-score data, to two places.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5,6,7], [2,3,3,5,5,7,8]), 2))", out: "0.98" },
    { task: "The panel's Negative preset, computed rather than trusted.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5,6,7], [8,7,7,5,4,2,1]), 2))", out: "-0.98" },
    { task: "The panel's No-correlation preset — check it is not exactly zero.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5,6,7], [4,8,2,6,3,7,4]), 2))", out: "-0.04" },
    { task: "A perfect curve that r reports as nothing.", code: "import statistics\nprint(round(statistics.correlation([-3,-2,-1,0,1,2,3], [9,4,1,0,1,4,9]), 2))", out: "0.0" },
    { task: "Show that changing the units leaves r alone.", code: "import statistics\nh = [1,2,3,4,5,6,7]\ns = [2,3,3,5,5,7,8]\nprint(round(statistics.correlation(h, [v * 100 for v in s]), 4) == round(statistics.correlation(h, s), 4))", out: "True" },
    { task: "Show that changing the units does not leave the covariance alone.", code: "import statistics\nh = [1,2,3,4,5,6,7]\ns = [2,3,3,5,5,7,8]\nprint(round(statistics.covariance(h, s), 2), round(statistics.covariance(h, [v * 100 for v in s]), 2))", out: "4.67 466.67" },
    { task: "The line of best fit through the same data.", code: "import statistics\nslope, intercept = statistics.linear_regression([1,2,3,4,5,6,7], [2,3,3,5,5,7,8])\nprint(round(slope, 3), round(intercept, 3))", out: "1.0 0.714" },
    { task: "r squared — the share of the variation the line explains.", code: "import statistics\nprint(round(statistics.correlation([1,2,3,4,5,6,7], [2,3,3,5,5,7,8]) ** 2, 4))", out: "0.9515" },
  ]},

  { t: "mistakes", items: [
    { bad: "# r = 0.85, so X causes Y", why: "It does not follow. Y might cause X, or a third variable might drive both, or the pattern might be coincidence in a small sample. Establishing a cause needs an experiment or an argument about the mechanism, never the coefficient on its own.", fix: "# r = 0.85 -> worth investigating\n# design an experiment before claiming a cause" },
    { bad: "# r = 0.02, so these two are unrelated", why: "r near zero rules out a <b>straight-line</b> relationship and nothing else. A perfect parabola scores exactly 0.0. Always plot before concluding that nothing is there.", fix: "# r = 0.02 -> no LINEAR relationship\n# plot it before concluding anything more" },
    { bad: "best = max(features, key=lambda f: statistics.covariance(f, target))", why: "Covariance scales with the units of both variables, so this picks whichever feature happens to be measured in the largest numbers. Convert one column from hours to minutes and it wins without changing anything real.", fix: "best = max(features, key=lambda f: abs(statistics.correlation(f, target)))" },
    { bad: "strongest = max(r_values)   # r_values includes -0.93", why: "Taking the plain maximum treats −0.93 as weaker than +0.10, when it is in fact a far stronger relationship. Strength lives in the magnitude and direction lives in the sign, and ranking by strength means ranking by <code>abs(r)</code>.", fix: "strongest = max(r_values, key=abs)" },
  ]},

  { t: "debug", intro: "This picks the feature most worth keeping, out of two candidates. It runs cleanly and answers \"revision_min\" — but attendance is plainly the better predictor of the target. Read it before opening the fix.", code: "import statistics\n\ntarget = [2, 3, 3, 5, 5, 7, 8]\nattendance = [1, 2, 3, 4, 5, 6, 7]\nrevision_min = [200, 900, 400, 1100, 300, 1200, 700]\n\nfeatures = [(\"attendance\", attendance), (\"revision_min\", revision_min)]\nbest = max((statistics.covariance(f, target), name) for name, f in features)\nprint(best[1])", symptom: "prints revision_min, even though attendance tracks the target almost perfectly and revision_min barely tracks it at all", q: "Both candidates are scored by the same function, fairly. So what is that function actually ranking them on?", fix: "import statistics\n\ntarget = [2, 3, 3, 5, 5, 7, 8]\nattendance = [1, 2, 3, 4, 5, 6, 7]\nrevision_min = [200, 900, 400, 1100, 300, 1200, 700]\n\nfeatures = [(\"attendance\", attendance), (\"revision_min\", revision_min)]\nbest = max((abs(statistics.correlation(f, target)), name) for name, f in features)\nprint(best[1])", why: "The ranking is by <b>covariance</b>, and covariance carries the units of both columns multiplied together. <code>revision_min</code> is measured in hundreds of minutes, so its covariance with the target is 445.24 while attendance manages only 4.67 — a hundredfold advantage that comes entirely from the choice of unit.<br/><br/>The actual relationships run the other way. Attendance correlates with the target at <b>0.9754</b>; revision_min at <b>0.5056</b>. The better feature loses by a factor of a hundred because somebody recorded one column in minutes instead of hours.<br/><br/>Nothing crashes, the output is a perfectly plausible column name, and on a real dataset with forty features you would never notice. This is the reason correlation exists as a separate function: <b>covariance tells you the direction, and only correlation lets you compare two relationships against each other</b>. The <code>abs</code> in the fix matters too — without it a strongly negative feature at −0.97 would rank below a useless one at +0.01." },

  { t: "recap", items: [
    "<b>Covariance</b> gives the direction two variables move in, but its size depends on the units",
    "<b>Correlation (r)</b> divides that by both standard deviations, fixing it between −1 and +1",
    "Sign is direction and magnitude is strength — <code>−0.9</code> is strong, not weak; rank by <code>abs(r)</code>",
    "<b>r only sees straight lines</b>: a perfect parabola scores exactly 0.0, so plot before concluding",
    "<b>Correlation is not causation</b> — look for the confounder driving both columns",
  ]},

  { t: "interview", items: [
    { level: "beginner", q: "What is the difference between covariance and correlation?", a: "Both measure whether two variables move together. Covariance keeps the units of both variables multiplied together, so its size is meaningless on its own and cannot be compared between pairs. Correlation divides by both standard deviations, which cancels the units and pins the result between −1 and +1 — so it is the one you use whenever you need to compare relationships." },
    { level: "beginner", q: "What does r = −0.85 tell you?", a: "A strong relationship in which one variable falls as the other rises. The minus sign is direction, not weakness: −0.85 is a much stronger relationship than +0.3. It also implies the points lie close to a straight downward line, since r only measures linear fit." },
    { level: "intermediate", q: "Two features in your training data correlate at 0.97. Why is that a problem?", a: "That is multicollinearity. The two columns carry nearly the same information, so a linear model cannot tell which of them deserves the credit — the coefficients become unstable and can flip sign on a small change in the data, which destroys any interpretation you wanted from them. Predictions often survive, but explanations do not. The usual responses are dropping one, combining them, or moving to a regularised model." },
    { level: "intermediate", q: "A colleague reports r = 0.02 and concludes the two variables are unrelated. Is that sound?", a: "Not as stated. r near zero only rules out a linear relationship. A perfect parabola gives exactly 0.0, and U-shaped or threshold effects are common in real data — dosage, pricing, load. I would plot it before accepting the conclusion, and if I suspected a monotonic but curved relationship I would also check Spearman's rank correlation, which reads order rather than straightness." },
    { level: "advanced", q: "One feature correlates with your target at 0.99. Your manager is delighted. What do you actually do?", a: "Get suspicious, because that is far more often leakage than luck. A near-perfect correlation usually means the answer has reached the inputs: a column computed from the target, a field populated after the outcome was known, or a proxy for it like a case-closed timestamp. I would check when that column is written relative to the moment of prediction — if it is not available at prediction time, the model is useless however good the validation score looks. The other checks are whether it is a near-duplicate of the target under another name, and whether the relationship survives on data from a different time period. Only once all three pass would I treat 0.99 as a genuine finding." },
  ]},
];
const S7 = [
  { t: "objectives", items: [
    "Read <b>P(A | B)</b> as \"the world where B already happened\"",
    "Apply <b>Bayes' theorem</b> to update a belief when evidence arrives",
    "Explain why a 99% accurate test can still be wrong most of the time",
    "Spot the <b>base rate</b> hiding behind a confident-sounding number",
  ]},
  { t: "hook", q: "A disease affects 1 person in 100. A test catches 99% of those who have it and wrongly flags only 5% of those who do not. You test positive. What is the chance you are ill?", why: "Almost everyone answers somewhere between 90% and 99%, and doctors do no better — this exact question has been put to them in studies and most get it wrong. The answer is about <b>17%</b>.<br/><br/>Test ten thousand people. A hundred are genuinely ill and the test finds 99 of them. But 9,900 are healthy, and 5% of those — <b>495 people</b> — are flagged anyway. So 594 positive results come back, and only 99 of them are real. Being wrong 5% of the time is cheap until you apply it to a very large group of healthy people, at which point the false alarms outnumber the real cases five to one.<br/><br/>Nothing is wrong with the test. What is missing from the intuition is how rare the disease was to begin with." },
  { t: "def", term: "Conditional probability", en: "P(A | B) is the probability of A given that B has already happened — computed by shrinking the world down to only the outcomes where B is true, and asking what share of those also have A.", hi: "The vertical bar reads as \"given\". It is not division and not \"and\": P(A | B) and P(B | A) are different questions with different answers, and swapping them is the mistake this lesson keeps returning to." },
  { t: "note", variant: "key", html: "💼 <b>On the job:</b> this is the arithmetic behind spam filters, fraud detection and every medical screening programme. It is also why <b>precision</b> and <b>recall</b> are reported separately in machine learning — a fraud model that catches 99% of fraud sounds excellent until you notice that fraud is 0.1% of transactions, and the alerts your team has to work through are almost all false. Any time somebody quotes an accuracy figure on a rare event, the base rate is the question to ask next." },

  { t: "h2", n: "1", text: "Conditioning shrinks the world" },
  { t: "p", html: "\"Given B\" means: throw away every outcome where B did not happen, then count what is left. The dice grid from the probability lesson shows it directly." },
  { t: "code", file: "cond.py", code: "outcomes = [(a, b) for a in range(1, 7) for b in range(1, 7)]\n\ngiven = [o for o in outcomes if o[0] == 4]\nboth = [o for o in given if o[0] + o[1] == 7]\n\nprint(len(given))\nprint(len(both))\nprint(round(len(both) / len(given), 4))", output: "6\n1\n0.1667" },
  { t: "p", html: "Knowing the first die is a 4 cuts the world from 36 outcomes to 6. Inside that smaller world exactly one gives a total of 7, so the answer is 1/6.<br/><br/>Here the condition changed nothing — the unconditional chance of a total of 7 is also 1/6. That is what <b>independent</b> means. Conditioning is only interesting when it does move the answer, which is the case in every remaining section of this lesson." },

  { t: "h2", n: "2", text: "Bayes' theorem" },
  { t: "p", html: "Bayes turns the conditional you have into the conditional you want. You know how the test behaves on ill people; you want to know what a positive result says about <i>you</i>.<br/><br/><code>P(ill | positive) = P(positive | ill) × P(ill) / P(positive)</code>" },
  { t: "p", html: "The only awkward term is the denominator, and it is not given to you — it has to be built. A positive result can arrive down two separate roads: a genuinely ill person correctly flagged, or a healthy person wrongly flagged. Add both." },
  { t: "code", file: "bayes.py", code: "prev = 0.01   # how common the illness is\nsens = 0.99   # caught, given ill\nspec = 0.95   # correctly cleared, given healthy\n\np_pos = sens * prev + (1 - spec) * (1 - prev)\n\nprint(round(p_pos, 4))\nprint(round(sens * prev / p_pos, 4))", output: "0.0594\n0.1667" },
  { t: "p", html: "About 5.9% of everyone tested comes back positive, and only 16.67% of those people are ill. Note that <code>(1 - spec) * (1 - prev)</code> is the false-alarm road: the 5% error rate multiplied by the 99% of people who are healthy. Leaving out that <code>(1 - prev)</code> is the commonest way to get this wrong, and §Find the bug is exactly that slip." },

  { t: "h2", n: "3", text: "The same answer, in people instead of decimals" },
  { t: "p", html: "Probabilities are hard to feel. Counts are not. This is the identical calculation with the decimals multiplied out into a crowd." },
  { t: "code", file: "counts.py", code: "pop = 10000\nsick = round(pop * 0.01)\nwell = pop - sick\n\ntp = round(sick * 0.99)   # ill and flagged\nfp = round(well * 0.05)   # healthy and flagged anyway\n\nprint(sick, well)\nprint(tp, fp)\nprint(round(tp / (tp + fp) * 100, 1))", output: "100 9900\n99 495\n16.7" },
  { t: "viz", name: "bayes-grid" },
  { t: "p", html: "The top bar is everyone tested; the bottom bar is only those who came back positive, which is the bar that matters. At the default settings the red slice is 99 people and the rest is 495 false alarms.<br/><br/>Now drag <b>how common the illness is</b> upward. At 5% the two are almost level. At 10% the positive result finally means more likely than not. Nothing about the test changed — the same 99% and 95% throughout — only the size of the healthy group the 5% error is being applied to. Then put prevalence back to 1% and drag <b>specificity</b> from 95% to 99%: the false alarms fall from 495 to 99 and the answer jumps to about 50%. That is the lever that actually matters on a rare condition, and it is not the one people ask about." },

  { t: "h2", n: "4", text: "Why the base rate wins" },
  { t: "p", html: "Hold the test fixed and vary only how common the illness is." },
  { t: "code", file: "baserate.py", code: "for prev in (0.001, 0.01, 0.1, 0.5):\n    num = 0.99 * prev\n    print(prev, round(num / (num + 0.05 * (1 - prev)) * 100, 1))", output: "0.001 1.9\n0.01 16.7\n0.1 68.8\n0.5 95.2" },
  { t: "p", html: "One test, four worlds. A positive result means a 1.9% chance in the first and a 95.2% chance in the last. The evidence is identical every time; what changes is what you believed before you saw it.<br/><br/>That is the real content of Bayes' theorem: <b>evidence updates a prior belief, it does not replace it</b>. A confident-sounding result applied to a rare thing stays unlikely, because it started out very unlikely indeed." },
  { t: "note", variant: "warn", html: "<b>P(A | B) is not P(B | A).</b> \"99% of ill people test positive\" and \"99% of positive people are ill\" are different sentences with wildly different truth values — 99% and 17% here. In a courtroom this swap has a name, the prosecutor's fallacy, and it has convicted innocent people: \"only 1 in a million share this profile\" is not \"1 in a million chance they are innocent\"." },

  { t: "think", q: "The test never changed. So why does a positive result mean something different in a screening programme than in a clinic?", a: "Because the two are testing different crowds, and the crowd is half the calculation.<br/><br/>A <b>screening programme</b> tests everybody, so nearly everyone it tests is healthy. The 5% false-positive rate gets applied to that enormous healthy group and generates far more false alarms than there are real cases to find.<br/><br/>A <b>clinic</b> tests people who arrived with symptoms. Among them the illness might be 30% common rather than 1%, so the same test on the same day with the same accuracy now yields a positive result that is right about 90% of the time.<br/><br/>The number that changed is the prior, and it came from who walked through the door rather than from any property of the test. This is also why doctors order a confirmatory test after a positive screen: the first result raises the prior enough that the second one becomes informative." },
  { t: "analogy", concept: "Bayes' theorem", real: "A very rare surname", html: "A witness says the person they saw was called Chatterjee. In a room of ten thousand people where a hundred are Chatterjees, that testimony is powerful — it narrows the field to about a hundred.<br/><br/>Now suppose the witness is right 99% of the time. Out of the hundred real Chatterjees they correctly name 99. But out of the 9,900 others they mistakenly say \"Chatterjee\" for 5% of them, which is 495 people. Their evidence has produced 594 names and 495 of them are wrong.<br/><br/>The witness is genuinely reliable. The surname is genuinely rare. Both are true, and together they still leave you with a list that is mostly wrong — because reliability is measured per person and false alarms are collected across everybody." },

  { t: "trace", intro: "The same calculation at a 2% base rate. Work out what each name holds once the line has run.", code: "prev = 0.02\nsens = 0.99\nspec = 0.95\np_pos = round(sens * prev + (1 - spec) * (1 - prev), 4)\npost = round(sens * prev / p_pos, 4)\nper_100 = round(post * 100, 1)", steps: [
    { q: "After line 4, <code>p_pos</code> is", answer: "0.0688", why: "Two roads to a positive result, added together: 0.99 × 0.02 = 0.0198 from the genuinely ill, and 0.05 × 0.98 = 0.049 from healthy people wrongly flagged. Note the false alarms are already the larger of the two." },
    { q: "After line 5, <code>post</code> is", answer: "0.2878", why: "0.0198 divided by 0.0688. Doubling the base rate from 1% to 2% took the answer from about 17% to about 29% — the evidence did not change at all, only the prior it was applied to." },
    { q: "After line 6, <code>per_100</code> is", answer: "28.8", why: "The same number said out loud: of every hundred people who test positive here, about 29 are genuinely ill and 71 are not. Converting to counts is worth doing every time, because a decimal like 0.2878 slides past unexamined in a way that \"71 of these people are fine\" does not." },
  ]},

  { t: "drills", intro: "One idea each. Write it yourself before you open the answer.", items: [
    { task: "Count the two-dice outcomes where the first is 4 and the total is 7.", code: "outcomes = [(a, b) for a in range(1, 7) for b in range(1, 7)]\nprint(len([o for o in outcomes if o[0] == 4 and o[0] + o[1] == 7]))", out: "1" },
    { task: "That as a conditional probability, given six outcomes in the shrunken world.", code: "print(round(1 / 6, 4))", out: "0.1667" },
    { task: "The true-positive road: caught, and ill.", code: "print(round(0.99 * 0.01, 4))", out: "0.0099" },
    { task: "The false-alarm road: flagged, and healthy.", code: "print(round(0.05 * 0.99, 4))", out: "0.0495" },
    { task: "Divide the first road by both roads together.", code: "print(round(0.0099 / (0.0099 + 0.0495), 4))", out: "0.1667" },
    { task: "The same test where half the population is ill.", code: "print(round(0.99 * 0.5 / (0.99 * 0.5 + 0.05 * 0.5) * 100, 1))", out: "95.2" },
    { task: "How many of the ten thousand are healthy.", code: "print(10000 - round(10000 * 0.01))", out: "9900" },
    { task: "How many of those healthy people are flagged anyway.", code: "print(round(9900 * 0.05))", out: "495" },
    { task: "How many of the ill are correctly caught.", code: "print(round(100 * 0.99))", out: "99" },
    { task: "How many ill people the test misses entirely.", code: "print(round(100 - 100 * 0.99))", out: "1" },
    { task: "A really rare condition: 1 in 1000, same test.", code: "print(round(0.99 * 0.001 / (0.99 * 0.001 + 0.05 * 0.999) * 100, 2))", out: "1.94" },
  ]},

  { t: "mistakes", items: [
    { bad: "# 99% of ill people test positive,\n# so 99% of positives are ill", why: "These are P(positive | ill) and P(ill | positive), and they are different questions. The first is a property of the test; the second depends on how common the illness is. Here they are 99% and 17%.", fix: "# P(positive | ill) = 0.99  -> about the test\n# P(ill | positive) = 0.17  -> about you" },
    { bad: "p_pos = sens * prev", why: "This counts only the ill people who tested positive and forgets that healthy people generate positives too. On a rare condition the forgotten road is the <b>bigger</b> one — 495 against 99 — so the answer comes out near certainty instead of near a sixth.", fix: "p_pos = sens * prev + (1 - spec) * (1 - prev)" },
    { bad: "# the test is 99% accurate,\n# so the base rate does not matter", why: "The base rate is half the calculation. The same test gives 1.9% at a 1-in-1000 prevalence and 95.2% at 50%. Accuracy describes the instrument; the prior describes the world it is pointed at.", fix: "# always ask: how common is this to begin with?" },
    { bad: "model_accuracy = 0.99   # fraud detector, shipped", why: "With fraud at 0.1% of transactions, a model that predicts \"not fraud\" every single time scores 99.9% accuracy and catches nothing. Accuracy is close to meaningless on rare events, which is exactly why precision and recall are reported instead.", fix: "precision = tp / (tp + fp)\nrecall = tp / (tp + fn)" },
  ]},

  { t: "debug", intro: "This is meant to answer the hook: given a positive test, what is the chance of being ill? It runs cleanly and reports 0.9519 — about 95%, which is the answer most people's intuition gives. The true answer is about 0.1667. Read it before opening the fix.", code: "prev = 0.01\nsens = 0.99\nspec = 0.95\n\ndef posterior(prev, sens, spec):\n    p_pos = sens * prev + (1 - spec) * prev\n    return round(sens * prev / p_pos, 4)\n\nprint(posterior(prev, sens, spec))", symptom: "reports 0.9519 instead of 0.1667 - the false-alarm term is being applied to the wrong group of people", q: "The false-alarm rate is right at 5%. So which people should that 5% be multiplied by?", fix: "prev = 0.01\nsens = 0.99\nspec = 0.95\n\ndef posterior(prev, sens, spec):\n    p_pos = sens * prev + (1 - spec) * (1 - prev)\n    return round(sens * prev / p_pos, 4)\n\nprint(posterior(prev, sens, spec))", why: "The false-alarm term is <code>(1 - spec) * prev</code> when it should be <code>(1 - spec) * (1 - prev)</code>. One character, and the entire meaning inverts.<br/><br/>False positives happen to <b>healthy</b> people. Multiplying the 5% error rate by <code>prev</code> applies it to the ill, who are 1% of the population — so the calculation imagines 5 false alarms where there are really 495. With the false-alarm road shrunk by a factor of a hundred, the true positives dominate and the answer sails up to 95%.<br/><br/>What makes this genuinely dangerous is that <b>95% is the answer intuition expects</b>. A bug whose output contradicts your expectation gets found in minutes; a bug that confirms it can survive to production and into a report somebody acts on. The 0.1667 is the surprising number, and surprise is the thing a wrong result here removes.<br/><br/>The habit that catches it: after computing P(positive), multiply it back into the population and check the count is believable. 0.0104 × 10,000 is 104 positive results from a test that flags 5% of nearly ten thousand healthy people — which cannot possibly be right." },

  { t: "recap", items: [
    "<b>P(A | B)</b> shrinks the world to outcomes where B happened, then counts A inside it",
    "<b>Bayes</b>: <code>P(A|B) = P(B|A) × P(A) / P(B)</code>, and P(B) must include every road to B",
    "On a rare condition, false alarms outnumber real cases — 495 against 99 here",
    "The <b>base rate</b> is half the answer: the same test gives 1.9% or 95.2% depending on it",
    "<b>P(A|B) ≠ P(B|A)</b> — swapping them is the prosecutor's fallacy",
  ]},

  { t: "interview", items: [
    { level: "beginner", q: "What does P(A | B) mean?", a: "The probability of A given that B has already happened. You restrict attention to the outcomes where B is true and ask what fraction of those also have A. It is not the same as P(A and B), and crucially it is not the same as P(B | A)." },
    { level: "beginner", q: "State Bayes' theorem and say what each part is for.", a: "P(A|B) = P(B|A) × P(A) / P(B). P(A) is the prior — what you believed before the evidence. P(B|A) is the likelihood — how well the evidence fits that belief. P(B) is the total probability of seeing the evidence at all, summed over every way it could arise, and it is the part people forget to build." },
    { level: "intermediate", q: "A test is 99% accurate for a disease affecting 1 in 100 people. You test positive. What is the chance you have it?", a: "About 17%, not 99%. Out of 10,000 people, 100 are ill and 99 are caught; of the 9,900 healthy, 5% are wrongly flagged, which is 495. So 594 positives, of which 99 are real — roughly one in six. The false alarms dominate because the healthy group is a hundred times larger, and no amount of test accuracy changes that on its own." },
    { level: "intermediate", q: "Why is accuracy a poor metric for a fraud model?", a: "Because fraud is rare. If 0.1% of transactions are fraudulent, a model that always predicts \"not fraud\" is 99.9% accurate and completely useless. Precision and recall separate the two things that matter — what share of your alerts are real, and what share of the real cases you caught — and on imbalanced data those are the numbers to report." },
    { level: "advanced", q: "Your screening test is 99% sensitive and 95% specific. Which number would you improve first, and why?", a: "Specificity, if the condition is rare. Sensitivity acts on the small ill group, so pushing it from 99% to 99.9% converts a single missed case. Specificity acts on the huge healthy group: going from 95% to 99% on 9,900 healthy people cuts false alarms from 495 to 99 and lifts the chance a positive is real from 17% to about 50%. The general rule is that the error rate applied to the larger group is the one worth attacking — which is also why a cheap sensitive screen followed by a specific confirmatory test beats trying to build one perfect test." },
  ]},
];
const S8 = [
...
export const QUIZZES = {
  "bayes": [
    ...
    { level: "easy", q: "What does P(A | B) mean?", options: ["The probability of A, given that B has already happened", "The probability that both A and B happen", "The probability of A divided by the probability of B", "The probability of B, given A"], correct: 0, why: "Conditioning shrinks the world to the outcomes where B is true, then asks what share of those also have A." },
    { level: "easy", q: "Is P(A | B) the same as P(B | A)?", options: ["Yes, always", "No — they are different questions with different answers", "Only when A and B are independent", "Only when both are above 0.5"], correct: 1, why: "\"99% of ill people test positive\" and \"99% of positive people are ill\" are 99% and 17% in this lesson's example. Swapping them is the prosecutor's fallacy." },
    { level: "easy", q: "In Bayes' theorem, what must the denominator P(B) account for?", options: ["Only the cases where A is true", "Only the false positives", "Every route by which B could occur, true positives and false positives alike", "The size of the sample"], correct: 2, why: "A positive test arrives either from an ill person correctly flagged or a healthy person wrongly flagged. Both roads have to be added, and forgetting the second is the classic error." },
    // Medium — apply it
    { level: "medium", q: "A disease affects 1 in 100. The test catches 99% of cases and wrongly flags 5% of healthy people. You test positive. Roughly what is the chance you are ill?", options: ["99%", "95%", "50%", "About 17%"], correct: 3, why: "Of 10,000 people: 99 true positives against 495 false alarms, so 99 out of 594. The healthy group is a hundred times larger, so its 5% error rate produces far more positives than the real cases." },
    { level: "medium", q: "Out of 9,900 healthy people, how many does a 95%-specific test wrongly flag?", options: ["495", "99", "9,405", "50"], correct: 0, why: "5% of 9,900. That single number is why a positive result on a rare condition means so much less than it sounds — the false alarms outnumber the 99 real cases five to one." },
    { level: "medium", q: "The same test is used where half the population is genuinely ill. What does a positive result mean now?", options: ["Still about 17%", "About 95%", "Exactly 99%", "It cannot be computed"], correct: 1, why: "The evidence has not changed at all; the prior has. With the ill and healthy groups the same size, the 99% catch rate easily outweighs the 5% false-alarm rate." },
    { level: "medium", q: "Why is accuracy a poor metric for a fraud detector?", options: ["Accuracy cannot be computed on imbalanced data", "Because fraud models are always overfitted", "Fraud is rare, so predicting \"not fraud\" every time already scores 99.9%", "Because accuracy ignores the training set"], correct: 2, why: "A metric that a useless model can max out is not measuring anything you care about. Precision and recall separate what share of alerts are real from what share of real cases were caught." },
    // Hard — edge cases and bugs; not written in the lesson
    { level: "hard", q: "Someone computes p_pos = sens * prev and stops there. What has gone wrong?", options: ["The false positives from the healthy group are missing, and on a rare condition they are the larger road", "Nothing — that is the correct formula", "The sensitivity should have been squared", "It double-counts the ill people"], correct: 0, why: "That denominator counts only ill people who tested positive. Since the healthy group generates 495 positives against 99 real ones, dropping it sends the answer towards certainty instead of towards a sixth." },
    { level: "hard", q: "A screening test is 99% sensitive and 95% specific for a rare condition. Which number should you improve first?", options: ["Sensitivity, because catching every case matters most", "Neither — the base rate is the only thing that matters", "Both equally", "Specificity, because it is applied to the far larger healthy group"], correct: 3, why: "Sensitivity acts on the tiny ill group; raising it converts about one missed case. Specificity acts on 9,900 healthy people — taking it from 95% to 99% cuts false alarms from 495 to 99 and lifts a positive from 17% to about 50%." },
    { level: "hard", q: "In court: \"only 1 in a million people match this profile, so there is a 1 in a million chance the defendant is innocent.\" What is wrong?", options: ["The profile statistic must be wrong", "It swaps P(match | innocent) for P(innocent | match), ignoring how many people could have matched", "Nothing — the reasoning is sound", "One in a million is too small a probability to use"], correct: 1, why: "In a city of ten million, roughly ten people match by chance. The evidence narrows the field to about ten, which is very far from establishing that this particular one is guilty. This is the prosecutor's fallacy and it has produced real wrongful convictions." },
  ],
  "correlation": [
    // Easy — did the core idea land?
    { level: "easy", q: "What range can the correlation coefficient r take?", options: ["−1 to +1", "0 to 1", "0 to 100", "Any number"], correct: 0, why: "Dividing the covariance by both standard deviations cancels the units and pins the result inside that range, whatever the two variables were measured in." },
    { level: "easy", q: "What does r = −0.9 tell you?", options: ["A weak relationship", "No relationship at all", "A strong relationship where one falls as the other rises", "An error, since r cannot be negative"], correct: 2, why: "The sign carries direction and the magnitude carries strength. −0.9 is far stronger than +0.3, which is the ranking beginners most often get backwards." },
    { level: "easy", q: "A strong correlation between two variables proves that one causes the other.", options: ["True, if r is above 0.9", "False — a third variable may drive both, or the direction may run the other way", "True, provided the sample is large", "True only for positive correlations"], correct: 1, why: "Correlation is a question, not an answer. Establishing a cause needs an experiment or an argument about the mechanism." },
    // Medium — apply it
    { level: "medium", q: "Why is correlation usually reported instead of covariance?", options: ["Covariance carries the units of both variables, so its size cannot be compared between pairs", "Covariance is harder to compute", "Covariance cannot be negative", "Covariance only works on large datasets"], correct: 0, why: "Convert one column from hours to minutes and its covariance jumps a hundredfold while nothing real has changed. Correlation divides that dependence out." },
    { level: "medium", q: "For x = [-3,-2,-1,0,1,2,3] and y = x squared, what is r?", options: ["1.0, because y is perfectly determined by x", "About 0.5", "It cannot be computed", "0.0"], correct: 3, why: "The relationship is perfect but curved, and r only measures straight-line fit. The rising and falling halves cancel exactly. This is why r = 0 means no LINEAR relationship, not no relationship." },
    { level: "medium", q: "Fire engines sent and rupees of damage correlate at 0.99. What is the best explanation?", options: ["Fire engines cause damage", "Both are driven by a third variable — the size of the fire", "The correlation must have been computed wrongly", "Damage causes fire engines to be dispatched, so the arrow is simply reversed"], correct: 1, why: "That third variable is a confounder. Big fires get more engines and big fires do more damage, so the two move together with neither causing the other." },
    { level: "medium", q: "Two input features in your training data correlate at 0.97. Why does that matter?", options: ["It makes the model train more slowly", "It guarantees the model will overfit", "Multicollinearity — the model cannot tell which one deserves the credit, so the coefficients become unstable", "It has no effect on anything"], correct: 2, why: "Predictions often survive it; explanations do not. The coefficients can flip sign on a small change in the data, which destroys any interpretation you wanted from them." },
    // Hard — edge cases and bugs; not written in the lesson
    { level: "hard", q: "You rank features with max(r_values), and one of them is −0.93. What goes wrong?", options: ["−0.93 is treated as weaker than +0.10, when it is a much stronger relationship", "Nothing — max handles negative numbers correctly", "The code raises a ValueError", "All the negative values are silently dropped"], correct: 0, why: "Strength lives in the magnitude and direction lives in the sign, so ranking by strength means ranking by abs(r). max(r_values, key=abs) is the fix." },
    { level: "hard", q: "A feature correlates with your target at 0.99. What is the most likely explanation?", options: ["You have found an unusually good predictor", "The dataset is too small to trust", "The target was measured incorrectly", "Leakage — information about the answer has reached your inputs"], correct: 3, why: "Usually a column computed from the target, or one populated only after the outcome was known. The test is whether that column is available at the moment you would need to predict; if not, the model is useless however good the score looks." },
    { level: "hard", q: "A colleague reports r = 0.02 and concludes the two variables are unrelated. What would you say?", options: ["They are right, 0.02 is close enough to zero", "It only rules out a linear relationship — a U-shaped or threshold effect would also give near-zero r", "r should have been squared first", "0.02 is actually a strong correlation"], correct: 1, why: "Plot it before accepting the conclusion. If you suspect a monotonic but curved relationship, Spearman's rank correlation reads order rather than straightness and would catch what Pearson's r misses." },
  ],
  "percentiles-iqr": [
    // Easy — did the core idea land?
    { level: "easy", q: "You finished at the 90th percentile. What does that mean?", options: ["About 90% of the field scored below you", "You scored 90 marks out of 100", "You were 90th in the list", "You scored 90% of the top score"], correct: 0, why: "A percentile is a position in the queue, not a score. The raw mark behind a 90th percentile depends entirely on how the rest of the field did." },
    { level: "easy", q: "Q2 is another name for which statistic?", options: ["The mean", "The median", "The mode", "The range"], correct: 1, why: "Q2 is the 50th percentile, which is exactly the definition of the median. Half the data lies below it." },
    { level: "easy", q: "What does the IQR measure?", options: ["The distance from the smallest to the largest value", "The average distance from the mean", "The width of the middle 50% of the data", "The number of outliers"], correct: 2, why: "It is Q3 minus Q1, so it covers the middle half of the data once the top and bottom quarters have been set aside." },
    // Medium — apply it
    { level: "medium", q: "How many values does statistics.quantiles(data, n=4) return?", options: ["1", "4", "2", "3"], correct: 3, why: "Four groups need three cut points between them: Q1, Q2 and Q3. The count is always n-1, which is why unpacking into two names raises a ValueError." },
    { level: "medium", q: "Which pair barely moves when one extreme value is made ten times larger?", options: ["The median and the IQR", "The mean and the standard deviation", "The mean and the IQR", "All of them move equally"], correct: 0, why: "Quartiles read rank, not magnitude. The largest value is still the largest however far out you push it, so Q1 and Q3 do not notice — while the standard deviation can triple." },
    { level: "medium", q: "What is the upper fence in the 1.5 × IQR rule?", options: ["Q3 + IQR", "Q3 + 1.5 × IQR", "Q3 × 1.5", "The maximum value"], correct: 1, why: "And the lower fence is Q1 - 1.5 × IQR. Anything outside either is flagged for review — flagged, not deleted." },
    { level: "medium", q: "A salary column's mean sits far above its median. Which summary should you report?", options: ["The mean and the standard deviation", "Either pair, it makes no difference", "The median and the IQR", "Only the maximum"], correct: 2, why: "That gap is the signature of a right skew. The mean is dragged by the long tail and the standard deviation is inflated by it, so together they describe a salary nobody actually earns." },
    // Hard — edge cases and bugs; not written in the lesson
    { level: "hard", q: "Why do engineering teams report p95 latency instead of the average?", options: ["Because an average hides the slow tail, where users actually give up", "Because percentiles are quicker to compute", "Because the average cannot be computed on latency data", "Because p95 is always smaller than the mean"], correct: 0, why: "A service can average a healthy 200ms while one request in twenty takes four seconds. The mean looks fine and a slice of your traffic is having a terrible time; p95 reports what the worst-served users experience." },
    { level: "hard", q: "What happens when you run q1, q3 = statistics.quantiles(data, n=4)?", options: ["q1 and q3 are set correctly", "q3 silently becomes the median", "It returns None", "It raises a ValueError, because three values cannot unpack into two names"], correct: 3, why: "The call returns three cut points. This one at least fails loudly — the dangerous version of the same confusion is subtracting q3 - q2 and calling the result an IQR, which runs clean and reports roughly half the true width." },
    { level: "hard", q: "The 1.5 × IQR rule flags 40% of the rows in a column. What is the most likely explanation?", options: ["40% of the data is genuinely corrupt", "The column is heavily skewed, or mixes two different populations", "The multiplier should always be raised until fewer rows are flagged", "The IQR was computed on sorted data"], correct: 1, why: "The rule is calibrated so that roughly 0.7% of normal data trips it, so 40% is evidence about the distribution rather than about the rows. Plot it before deciding: a heavy skew, two populations sharing a column, or a unit change partway through are the usual causes." },
  ],
  "normal-distribution": [
    // Easy — did the core idea land?
