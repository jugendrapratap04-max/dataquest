...
const L12 = [
  ...
  { t: "trace", intro: "Three writes, then two reads. Work out each value before you open it.", code: "with open(\"notes.txt\", \"w\") as f:\n    f.write(\"alpha\\n\")\n\nwith open(\"notes.txt\", \"w\") as f:\n    f.write(\"beta\\n\")\n\nwith open(\"notes.txt\", \"a\") as f:\n    f.write(\"gamma\\n\")\n\nf = open(\"notes.txt\")\nrows = f.read().splitlines()\ncount = len(rows)\nagain = f.read()\nleft = len(again)\nf.close()\n\nwith open(\"notes.txt\", \"a\") as f:\n    f.write(\"delta\\n\")\n\ntotal = len(open(\"notes.txt\").read().splitlines())", steps: [
    { q: "After line 11, <code>rows</code> is", answer: "['beta', 'gamma']", why: "\"alpha\" never survives. The second <code>open(..., \"w\")</code> on line 4 emptied the file before writing \"beta\", and line 7 opened in <code>\"a\"</code>, which adds without wiping." },
...
export const QUIZZES = {
  ...
  "file-handling": [
    // Easy
    { level: "easy", q: "Which mode <b>adds</b> to a file without destroying what is already in it?", options: ["w", "a", "r", "x"], correct: 1, why: "<code>\"a\"</code> is append — it opens with the cursor at the end and leaves the existing contents alone. <code>\"w\"</code> would empty the file first." },
    { level: "easy", q: "What does <code>open(\"notes.txt\")</code> with no mode given do?", options: ["Opens it for writing", "Opens it for reading", "Creates an empty file", "Raises an error — a mode is required"], correct: 1, why: "The default mode is <code>\"r\"</code>, read. That is a deliberately safe default: the one thing it cannot do is damage the file." },
    ...
    // Medium
    { level: "medium", q: "<code>open(\"data.csv\", \"w\")</code> runs, and then the script crashes on the very next line. What happened to data.csv?", options: ["Nothing — nothing was written", "It is empty; opening in w truncated it immediately", "It is unchanged until close() is called", "Python rolls the change back"], correct: 1, why: "<code>\"w\"</code> truncates the file <b>as it opens</b>, not when you write. The crash is irrelevant — the data was already gone." },
    ...
    { level: "hard", q: "<code>with open(\"data.csv\", \"w\") as f:</code> then <code>rows = f.read()</code>. What goes wrong?", options: ["Only the read fails; the file is fine", "The file was already emptied, and the read fails too", "It works — w allows reading", "It returns an empty string harmlessly"], correct: 1, why: "Both things go wrong, and the order matters. <code>\"w\"</code> destroyed the file on open, and <b>then</b> the read raised <code>io.UnsupportedOperation</code> because a write-mode handle is not readable. The error you see is the harmless half; the damage already happened." },
  ],
};
