...
export const QUIZZES = {
  ...
  "file-handling": [
    ...
    { level: "easy", q: "What does <code>open(\"notes.txt\")</code> with no mode given do?", options: ["Opens it for writing", "Creates an empty file", "Raises an error — a mode is required", "Opens it for reading"], correct: 3, why: "The default mode is <code>\"r\"</code>, read. That is a deliberately safe default: the one thing it cannot do is damage the file." },
    { level: "easy", q: "Why is <code>with open(...) as f:</code> preferred over calling <code>open()</code> yourself?", options: ["It runs faster", "It closes the file even if the code inside crashes", "It makes the file readable", "It is the only way to write files"], correct: 1, why: "<code>with</code> guarantees the handle is closed on the way out, including when an exception is raised. A manual <code>close()</code> gets skipped by any error above it, and buffered writes are then never flushed to disk." },
    // Medium
    ...
    { level: "medium", q: "You call <code>f.read()</code> twice on the same open handle. What does the second call return?", options: ["An empty string", "The file contents again", "None", "An error"], correct: 0, why: "The first <code>read()</code> moved the cursor to the end of the file, and <code>read()</code> only returns what is ahead of the cursor. Use <code>f.seek(0)</code> to rewind, or just reopen the file." },
    { level: "medium", q: "A log function opens with <code>\"w\"</code> on every call. After three calls, what is in the file?", options: ["All three messages", "Only the first message", "Nothing", "Only the last message"], correct: 3, why: "Each call empties the file before writing its own line, so only the last one survives. Nothing errors — the history is simply lost. Open once in <code>\"w\"</code> at startup, then use <code>\"a\"</code>." },
    { level: "medium", q: "Which is the right way to process a 4 GB CSV line by line?", options: ["for line in f.readlines():", "for line in f:", "for line in f.read():", "for line in f.read().split()"], correct: 1, why: "Iterating the handle reads one line at a time and keeps memory flat. <code>readlines()</code> and <code>read()</code> both build the entire file in memory first, and on 4 GB the process is killed before the loop starts." },
    // Hard
    { level: "hard", q: "A file holds <code>alpha</code> and <code>beta</code>. You open it in <code>\"a\"</code> and write <code>gamma</code>. Where does gamma land?", options: ["At the start, pushing the others down", "It replaces beta", "At the end, after beta", "It replaces the whole file"], correct: 2, why: "Append mode parks the cursor at the end of the file when it opens, so every write goes after what is already there. That is the entire difference between <code>\"a\"</code> and <code>\"w\"</code>." },
    { level: "hard", q: "What does mode <code>\"x\"</code> do when the file already exists?", options: ["Raises FileExistsError", "Overwrites it", "Appends to it", "Creates a numbered copy"], correct: 0, why: "<code>\"x\"</code> is exclusive creation — it makes a new file or fails loudly. Use it when a silent overwrite would destroy something you cannot get back, so a name collision becomes a visible error instead of lost data." },
